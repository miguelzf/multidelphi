...
%type<ArrayList> labelidlst idlst varids
...
%type<int> mulop addop relop fordir
...
%type<PropertySpecifiers> spropspecsnormal spropspecsoverride spropspecsarray
    ...
    // Types
%type<VariableType> paramtypeopt paramtypespec funcpartype  // scalar or array
%type<VariableType> enumtype rangetype ordinaltype fixedtype stringtype
...
library
    : id SCOL usesopt maindecllst block     { $$ = new LibraryNode($1, $3, $4  , $5); }
    | id SCOL usesopt             block     { $$ = new LibraryNode($1, $3, null, $4); }
    ;
    
package
    : id SCOL requires contains             { $$ = new PackageNode($1, $3, $4); }
    ;
...
unit
    : id SCOL interfsec implsec
        initsec finalsec                    { $$ = new UnitNode($1, $3, $4, $5, $6); }
    | id SCOL interfsec implsec initsec     { $$ = new UnitNode($1, $3, $4, $5, null); }
    | id SCOL interfsec implsec finalsec    { $$ = new UnitNode($1, $3, $4, null, $5); }
    | id SCOL interfsec implsec             { $$ = new UnitNode($1, $3, $4); }
    ;
...
vardecl
    : varids vartype SCOL                       { $$ = CreateDecls($1, new CreateDecl((s) => new VarDeclaration(s, $2))); }
    | varids vartype KW_EQ constinit SCOL       { $$ = CreateDecls($1, new CreateDecl((s) => new VarDeclaration(s, $2, $4))); }
    | varids vartype KW_ABSOLUTE id SCOL        { $$ = CreateDecls($1, new CreateDecl((s) => new VarDeclaration(s, $2, $4))); }
    | varids proceduraltype                     { $$ = CreateDecls($1, new CreateDecl((s) => new VarDeclaration(s, $2))); }
    | varids proceduralsign SCOL
        funcdir_noterm_opt functypeinit SCOL    { $$ = CreateDecls($1, new CreateDecl((s) => new VarDeclaration(s, $2, $5))); $2.Directives = $4; }
    ;
...
proceduralsign
    ...
    | KW_PROCEDURE formalparams typeobj         { $$ = new MethodType($2); } 
    | KW_FUNCTION  formalparams funcret typeobj { $$ = new MethodType($2, $3); } 
    ;
...
stmt
    : nonlblstmt                        { $$ = $1; }
    | labelid COLON nonlblstmt          { $$ = new LabelStatement($1, $3); }
    ;
...
casestmt
    : KW_CASE expr KW_OF casesellst
        elsecase KW_END                 { $$ = new CaseStatement($2, $4, $5); }
    ;
...
casesellst
    ...
    | casesellst SCOL caseselector      { $$ = ListAdd<StatementList,Statement>($1, $3); }
    ;

caseselector
    :                                   { $$ = null; }
    | caselabellst COLON nonlblstmt { $$ = new CaseSelector($1, $3); }
    ;
...
whilestmt
    : KW_WHILE expr KW_DO nonlblstmt    { $$ = new WhileLoop($2, $4); }
    ;

forstmt
    : KW_FOR identifier KW_ASSIGN expr
        fordir expr KW_DO nonlblstmt    { $$ = new ForLoop($2, $4, $6, $8, $5); }
    ;
...
withstmt
    : KW_WITH exprlst KW_DO nonlblstmt  { $$ = new WithStatement($2, $4); }
    ;

tryexceptstmt
    : KW_TRY blockstmt KW_EXCEPT
        exceptionblock KW_END           { $$ = new TryExceptStatement($2, $4); }
    ;
...
ondef
    : KW_ON id COLON id KW_DO nonlblstmt SCOL   { $$ = new OnStatement($2, $4, $6); }
    | KW_ON          id KW_DO nonlblstmt SCOL   { $$ = new OnStatement(null, $2, $4); }
    ;

tryfinallystmt
    : KW_TRY blockstmt 
        KW_FINALLY blockstmt KW_END     { $$ = new TryFinallyStatement($2, $4); }
    ;
...
unaryexpr
    ...
    | lvalasval                         { $$ = $1; } 
    | set                               { $$ = $1; }
    | KW_ADDR lvalue                    { $$ = new AddressLvalue($2); }
...
exprlst
    ...
    | exprlst 
        COMMA expr  { $$ = ListAdd<ExpressionList,Expression>($1, $3); }
    ;
...
constexprlst
    ...
    // 1 or more exprs
arrayconst
    : LPAR constinit COMMA arrexprlst RPAR  { $$ = new ArrayConst($4); $4.InsertAt(0, $2); }
    ;

arrexprlst
    : constinit                             { $$ = new ExpressionList($1); }
    | arrexprlst COMMA constinit            { $$ = ListAdd<ExpressionList,Expression>($1, $3); }
    ;
...
objfield
    ...
    | idlst COLON proceduralsign        { $$ = CreateDecls($1, new CreateDecl((s) => new FieldDeclaration(s, $3))); }
    | idlst COLON proceduralsign
            SCOL funcdirectlst          { $$ = CreateDecls($1, new CreateDecl((s) => new FieldDeclaration(s, $3))); $3.Directives = $5; }
    ;
...
interftype
    : KW_INTERF heritage guid interfbody{ $$ = new InterfaceType($2, $4, $3); }
    | KW_INTERF heritage                { $$ = new InterfaceType($2); }
    ;

interfbody
    : interfcomplst KW_END              { $$ = new ObjectSection(null, $1, Scope.Public); }
    ;
...
property
    ...
    : KW_PROPERTY id COLON funcrettype
        spropspecsnormal SCOL                       { $$ = new PropertyDeclaration($2, $4, $5); }
    // Array properties
    | KW_PROPERTY id arrayprops COLON funcrettype
        spropspecsarray SCOL defaultdiropt          { $$ = new ArrayProperty($2, $5, $3, $6, $8); }
    // Property overrides
...
typedecl
    ...
    | idtypeopt proceduraltype      { $$ = new TypeDeclaration($1, $2); }
...
funcpartype
    ...
    | TYPE_ARRAY KW_OF funcpartype  { $$ = new ArrayType($3); }
    | stringtype                    { $$ = $1; }
    | TYPE_PTR                      { $$ = PointerType.Single; }
    ;
